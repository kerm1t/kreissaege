...
#include <WIndowsx.h>
#include <ZMouse.h>
int m_xStart;				// bei einer Zieh-Bewegung wird hier der Start gemerkt
int m_yStart;
int m_xPos;
int m_yPos;
int m_zDelta;
int m_lbutton, m_rbutton;	// 1 = gedrückt, 0 = nicht gedrückt
// <-- ws
...
//ws - mouse-Wheel auslesen
		case WM_MOUSEWHEEL:
			m_zDelta = (int)wParam;
			break;

//ws - mouse-Buttons auslesen
		case WM_LBUTTONDOWN:			// links
//			printf("prima!\n");
			m_lbutton = 1;
			break;
		case WM_LBUTTONUP:
...
//			printf("fertig!\n");
        ...
        case WM_KEYDOWN:
        {
			// In diesem Event werden auch Pfeiltasten erfasst,
			// ein "a" gibt den ASCII-Wert 65 zurück
			// hier kann also nicht zwischen Groß- und Kleinschreibung unterschieden werden

//			printf("Taste: %d\n",(int)wParam&0xFF);
            taste_ascii = (int)wParam&0xFF;
			// close on escape key
...
//ws
		...
		{
			// ein großes "A" gibt 65, ein kleines "a" 97 zurück
			taste = (int) wParam;
			break;
		}
//sw
...
//ws - wie kann ich wohl den Close-Button "verbergen"?
//    wnd = CreateWindow(title,title,WS_POPUP | WS_MINIMIZEBOX | WS_MAXIMIZEBOX,0,0,0,0,0,0,0,0);
#else
